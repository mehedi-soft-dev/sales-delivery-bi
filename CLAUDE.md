...
- **Revised again (discussed with the user): one Admin write path now exists — role→permission mapping.** `PUT /api/admin/roles/{roleId}/permissions` (body: full desired `permissionCodes` list) replaces a role's `sales.RolePermissions` rows via `IAdminRepository.SetRolePermissionsAsync`, gated by a new `AuthorizationPolicies.AdminWrite` policy requiring `admin.access.manage` (seeded to `SuperAdmin` only, same as `admin.access.view`) **on top of** the controller's class-level `AdminRead` — a caller needs both codes. `AdminAppService.UpdateRolePermissionsAsync` validates every submitted code against `PermissionCodes.All` (400 on an unknown code) before writing. `PermissionCodes` itself moved from `Infrastructure/Security` to `Application/Common` so the AppService can reference the same allow-list without Application depending on Infrastructure. **User/role CRUD and role→unit assignment remain explicitly out of scope** — this is intentionally the one piece, not a general admin-CRUD opening; any further Admin write endpoint needs its own discussion first.
- Full policy mapping + permission codes: `docs/plans/security/security-plan.md`.
- **Known limitation, accepted (discussed with the user): Merchandiser row-level scope is unit-level only, not per-merchandiser.** `docs/requirements/Sales_Delivery_Module_BI_Developer_Guidelines.md` §5 describes the Merchandiser role as "Own quotations + limited pipeline" — but `IUnitAccessGuard` only scopes by unit, and there's no link between `sales.Users` and `sales.Merchandisers` at all (no `User.MerchandiserId`, no reverse nav), so a Merchandiser-role login sees every quotation in their assigned unit(s), not just their own. Building real per-merchandiser scoping would mean a new FK + migration, a `merchandiser_id` JWT claim, extending `IUnitAccessGuard`/`UnitScope` (or a new guard) with a merchandiser predicate, a repository SQL filter, and folding it into `CacheKeys` — real surface on the access-control model this file calls out as non-negotiable (top of file). Deliberately not built for this demo; unit-level is the accepted ceiling for the Merchandiser role unless discussed again.
...
- **Auth login is now in scope, and Admin > Users/Roles/Permissions is now in scope as a read-only view (§5).** `User`/`Role`/`UserUnit`/`RolePermission` tables + `POST /api/auth/login` exist here — a deliberate, discussed reversal of the earlier "separate Identity service" plan, driven by the need for a real working login flow. The new Admin pages call the 3 `GET` endpoints (`/api/admin/users`, `/roles`, `/permissions`) plus, as of the role→permission mapping reversal below, one `PUT`. The 7 seed users, one per seeded role (`DatabaseSeeder`), remain the only way accounts get created, `IsDevelopment()`-guarded like the rest of the seed data.
- **Revised again (discussed with the user): role→permission mapping edit is now in scope (§5) — the Roles page has an "Edit" action, gated by `admin.access.manage`, that opens a dialog and calls `PUT /api/admin/roles/{roleId}/permissions`.** This was deliberately scoped down to just this one piece from a broader ask (full user/role CRUD + permission CRUD + role↔unit assignment). **Still explicitly out of scope: any UI or endpoint to create/edit/delete users or roles themselves, or to assign units to a role.** Don't treat this one reversal as opening the door to the rest — each further piece needs its own discussion.
