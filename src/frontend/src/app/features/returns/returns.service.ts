...
import type { DashboardResponse, ReturnResponseDto } from '../../core/models/dashboard.models';
...
const RETURNS_ENDPOINT = `${environment.apiBaseUrl}/sales/returns/summary`;

@Injectable({ providedIn: 'root' })
export class ReturnsService {
  private readonly http = inject(HttpClient);

  private readonly query = createQuerySignal<ReturnsFilter, ReturnResponseDto>((filter) => {
    ...
    return this.http.get<DashboardResponse<ReturnResponseDto>>(RETURNS_ENDPOINT, { params });
  });
  ...
  load(filter: ReturnsFilter): void {
