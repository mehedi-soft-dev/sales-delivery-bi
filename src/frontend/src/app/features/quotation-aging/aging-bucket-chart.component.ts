...
import type { AgingBucketDto } from '../../core/models/dashboard.models';
import { CHART_COLORS, createBaseChartOptions } from '../../shared/charts/apex-chart-theme';

/** Fixed display order — the API doesn't guarantee array order, so buckets are re-sorted client-side. */
const BUCKET_ORDER = ['0-7', '8-15', '16-30', '31-60', '60+'];

/** Combo chart: outstanding value (bars, left axis) + quotation count (line, right axis) per aging bucket. */
@Component({
  selector: 'app-aging-bucket-chart',
  imports: [ChartComponent],
  templateUrl: './aging-bucket-chart.component.html',
})
export class AgingBucketChartComponent {
  readonly buckets = input.required<AgingBucketDto[]>();
  ...
  private readonly orderedBuckets = computed(() => {
    const byBucket = new Map(this.buckets().map((entry) => [entry.bucket, entry]));
